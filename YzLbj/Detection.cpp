...
void removeBall(Mat &img_temp)
{
	...
	FillInternalContours(&img_temp2, 500);
	threshold(img_temp, img_temp, 128, 255, CV_THRESH_BINARY_INV);
}
...
void getBallLoc(Mat imgHole, vector<Rect> &holeList)
{
	...
	if (contours.size() > 0)
	{
		for (size_t i = 0; i < contours.size(); i++)
		{
			...
			//cout << i << ":=" << area << endl;
			if (contours[i].size() > 5 && area >= 60 && area < 400)//小球的面积阈值的取值是这个要通过多次测试
			{
				//cout << "area = " << area << endl;
				Rect boundRect;
				...
				//cout << "ratio = " << ratio << ",bigValue = "<<bigValue << endl;

				if (bigValue < 55 && smallValue >= 7 && ratio <= 6.5)//bigValue的值有可能是两颗小球，所以设的比较大，相应的ratio的值也较大
				{
	...
	{
#ifdef printResult
		cout << "此图里有" << holeList.size() << "个洞" << endl;
#endif
		//isHasHole = holeList.size();
#ifdef showResult
		imshow("result", dstImg_RGB);
		waitKey(10);
#endif
	}
	else
...
#ifdef showResult
	imwrite("imgROI_before.bmp", imgROI_before);
	imwrite("imgROI_after.bmp", imgROI_after);
#endif
...
#ifdef showResult
	imwrite("quexian.bmp", imgROI_after1);
#endif
	//纵向投影
...
#ifdef showResult
							rectangle(dstImg_RGB1, rect, Scalar(0, 255, 255), 1, CV_AA);
#endif
			...
			for (int i1 = 0; i1 < lineNumList.size(); i1++)
			{
				if (lineNumList[i1] == 0)
				{
					...
					else
					{
						if (i1 == lineNumList.size() - 1)
						{
							...
							//cout << endY - beginY << endl;

							if (endY - beginY <13) //小于20个像素的不算断裂，用15个像素表示20
							{
								symbol = 0;
								continue;
							}
				...
				else
				{
					if (symbol == 1)
					{
						...
						//cout << endY - beginY << endl;
						if (endY - beginY < 13) //小于20个像素的不算断裂
						{
							symbol = 0;
							continue;
						}
						//重新定位一下这个缺陷的宽度，因为有可能是磨损导致的破裂，这样取的缺陷位置太宽了
						Rect rect_temp;
						rect_temp.x = begin;
						rect_temp.y = max(beginY - 10, 0);
						rect_temp.width = end - begin;
						rect_temp.height = endY - beginY;
						Mat subImg = imgROI_after(rect_temp);
						int newX1 = 0;
						int newX2 = subImg.cols - 1;
						int t1 = 0;
						for (int t2 = 0; t2 < subImg.cols; t2++)
						{
							int value = 0;
							value = subImg.at<uchar>(t1, t2);
							if (value == 255)
							{
								newX1 = t2;
								break;
						...
						t1 = 0;
						for (int t2 = subImg.cols - 1; t2 >= 0; t2--)
						{
							int value = 0;
							value = subImg.at<uchar>(t1, t2);
							if (value == 255)
...
int getResult(Mat img, Rect rect)
{
...
#ifdef showResult
	Mat dstImg_RGB = Mat::zeros(imgROI_after.size(), CV_8UC3);
	cvtColor(imgROI_after, dstImg_RGB, CV_GRAY2BGR);
#endif
	//根据找到的缺失位置坐标，找横向附近有没有小球holeList即可
	if (result == 0)
	{
	...
	{
		if (missLineIndex.size() == 0) //整个篦条都缺失的情况
		{
			int missNum = 1;
			if (missNum > 0)
			{
				missNum = max(missNum, int(rect.width / 45.0));
			}
			ret = missNum;
			return ret;
		}
	...
	for (int i = 0; i < missLineIndex.size(); i++)
	{
		...
		{
#ifdef showResult
			rectangle(dstImg_RGB, missLineIndex[i].rect, Scalar(0, 0, 255), 1, CV_AA);
#endif
			if (i == 0)
			{
				missNum++;
			...
			{
				int symbol = 0;
				for (int i1 = 0; i1 < i; i1++) //因为一个篦条可能有2个断裂处（一个是伪的），这样会造成missNum计数错误
				{
					if (missLineIndex[i1].useful == 1)
					{
						if ((missLineIndex[i1].rect.x - missLineIndex[i].rect.x) < 15)
						{
							symbol = 1;
							break;
						}
	...
	if (missNum > 0)
	{
		missNum = max(missNum, int(rect.width / 55.0 + 0.5));
	}
	ret = missNum;
	return ret;
}
			...
			if (contours[i].size() > 5 && area > 1500 && area < 30000)
			{
				...
				if (rect_width > normalWidth && rect_height >= normalHeight)
				{
					...
					if (rect_width < 50) //单个空隙
					{
						...
						if (leftRect.height <= 0 || leftRect.width <= 0)
						{
							continue;
						}
					...
					else
					{
						...
						if ((Rect1.width + Rect1.x) >= src.cols || (Rect1.height + Rect1.y) >= src.rows)
						{
							continue;
						}
	...
	if (contours.size() > 0)
	{
		for (size_t i = 0; i < contours.size(); i++)
		{
			...
			if (contours[i].size() > 5 && area > effectiveArea && area < 30000)
			{
				...
				Mat contourImg = Mat::zeros(cvSize(width, height), CV_8UC1);
				drawContours(contourImg, contours, i, Scalar(255, 255, 255), -1, 8);
				...
				for (int t1 = 0; t1 < height; t1++)
				{
					...
					{
						int value = 0;
						value = roiImg.at<uchar>(t1, t2);
						int value1 = 0;
						value1 = contourImg.at<uchar>(t1, t2);
						if (value1 == 255)
						{
							sumvalue += value;
							num++;
						...
						{
							roiImg.at<uchar>(t1, t2) = 0;
						}
					}

				}
				float meanthresh = 0;
				meanthresh = sumvalue * 1.0 / max(1, num);
				//cout << "meanthresh = " << meanthresh << endl;
